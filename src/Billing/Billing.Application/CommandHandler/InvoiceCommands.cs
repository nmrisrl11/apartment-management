...
namespace Billing.Application.CommandHandler
{
    public class InvoiceCommands : IInvoiceCommands
    {
        ...
        public async Task<Result<InvoiceResponse>> AddAsync(Guid tenantId, Guid leasingAgreementId, CancellationToken cancellationToken)
        {
            // Todo: Add the checking of TenantId and LeasingAgreementId here
            Invoice invoiceToCreate = Invoice.Create(
                new TenantId(tenantId),
                new LeasingAgreementId(leasingAgreementId));
        ...
        public async Task<Result> AddInvoiceLineItemAsync(Guid invoiceId, string description, decimal quantity, Money unitPrice, CancellationToken cancellationToken)
        {
            try
            {
                ...
                invoice.AddLineItem(new InvoiceId(invoiceId), description, quantity, unitPrice);
                await _unitOfWork.SaveChangesAsync(cancellationToken);
        ...
        {
            Invoice? invoiceToDelete = await _unitOfWork.Invoices.GetByIdAsync(new InvoiceId(invoiceId));
